...
orgs.newOrg('iot.zenoh', 'eclipse-zenoh') {
  ...
  secrets+: [
    ...
    orgs.newOrgSecret('SSH_PASSPHRASE') {
      value: 'pass:bots/iot.zenoh/ssh/passphrase',
    },
    orgs.newOrgSecret('SSH_PRIVATE_KEY') {
      value: 'pass:bots/iot.zenoh/ssh/secret-subkeys.asc',
    },
  ...
  _repositories+:: [
    ...
    orgs.newRepo('zenoh-flat') {
      allow_auto_merge: true,
      allow_merge_commit: true,
      allow_update_branch: false,
      delete_branch_on_merge: false,
      dependabot_security_updates_enabled: true,
      description: 'Flat Rust API for Zenoh for automatic code generation with prebindgen tool',
      homepage: 'http://zenoh.io',
      web_commit_signoff_required: false,
      workflows+: {
        default_workflow_permissions: 'write',
    ...
    orgs.newRepo('zenoh-flat-jni') {
      ...
      delete_branch_on_merge: false,
      dependabot_security_updates_enabled: true,
      description: 'Common JNI library for zenoh-java and zenoh-kotlin autogenerated from zenoh-flat',
